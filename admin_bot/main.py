...
Key fixes applied:
  - functools.wraps on every decorator so telebot correctly registers handlers
  - bot.answer_callback_query() called at the START of every callback handler
  - CSV: BytesIO.name set for correct filename; delete_message wrapped in try/except
  - Never pass ReplyKeyboardMarkup to edit_message_text (Telegram API rejects it)
"""
...
# ─── Decorators (with functools.wraps so telebot sees unique handler names) ──
...
# ─── Safe user count (always inits DB first for fresh data) ──────────────────
...
def _safe_count(bot_username: str) -> dict:
    ...
    try:
        ...
        logger.warning(f"_safe_count failed for {bot_username}: {e}")
...
def handle_add_bot_token(message: types.Message):
    ...
    if db.add_child_bot(token, uname, name, uid):
        ...
        cs(uid)
        bot.send_message(
            message.chat.id, f"⚠️ @{uname} is already added.", reply_markup=admin_main_menu()
        )
...
def cb_confirm_delete(call: types.CallbackQuery):
    ...
    try:
        bot.edit_message_text(
            chat_id=call.message.chat.id,
            message_id=call.message.message_id,
            text=f"✅ <b>{row['bot_name']}</b> (@{row['bot_username']}) deleted.",
            parse_mode="HTML",
        )
    except Exception:
        pass
...
def cb_cancel_delete(call: types.CallbackQuery):
    ...
    try:
        bot.edit_message_text(
            call.message.chat.id, call.message.message_id, text="❌ Cancelled."
        )
    except Exception:
        pass
...
def _send_bot_list(chat_id: int, bots: list, page: int, msg_id: int = None):
    ...
    for i, b in enumerate(page_bots, 1):
        ...
        c    = _safe_count(b["bot_username"])          # live count, always fresh
        lines.append(
    ...
    if msg_id:
        try:
            bot.edit_message_text(
                chat_id=chat_id, message_id=msg_id, text=text, reply_markup=kb, parse_mode="HTML"
            )
        except Exception:
...
def cb_toggle_select(call: types.CallbackQuery):
    ...
    if row["is_running"]:
        _stop_child(bot_id)
        status_text = f"⏹ <b>{row['bot_name']}</b> (@{row['bot_username']}) — <b>Stopped</b>"
    else:
        ok, err = _start_child(bot_id, row)
        if ok:
            status_text = f"▶️ <b>{row['bot_name']}</b> (@{row['bot_username']}) — <b>Running</b>"
        else:
            status_text = f"❌ Failed to start <b>{row['bot_name']}</b>:\n<code>{err}</code>"
    try:
        bot.edit_message_text(
            chat_id=call.message.chat.id,
            message_id=call.message.message_id,
...
def menu_download_csv(message: types.Message):
    ...
    status_msg = bot.send_message(message.chat.id, "⏳ Generating CSV…")
    ...
    for b in bots:
        ...
        try:
            ...
            logger.warning(f"CSV export error for {b['bot_username']}: {e}")
            users = []
            count = 0

        writer.writerow([f"=== Bot: {b['bot_name']} (@{b['bot_username']}) ==="])
        writer.writerow([f"Total Users: {count}"])
        writer.writerow(["User ID", "Username", "Full Name", "Status", "Blocked", "Joined Date", "Last Seen"])
        for u in users:
            if u["is_blocked"]:
                status = "Blocked"
            elif u["is_active"]:
    ...
    # Build BytesIO with .name set — telebot uses .name as the filename
    csv_bytes = output.getvalue().encode("utf-8-sig")   # utf-8-sig = Excel-friendly BOM
    ts_file   = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    filename  = f"bot_users_{ts_file}.csv"
    buf       = io.BytesIO(csv_bytes)
    buf.name  = filename                                 # ← required for correct filename in Telegram

    # Safe delete of status message (don't crash if it fails)
    try:
...
def menu_restore(message: types.Message):
    ...
    bot.send_message(
        ...
        "Send or forward the backup <b>.zip</b> file.\n\n"
        "⚠️ All running bots will stop, data will be restored, then bots <b>auto-restart</b>.",
        reply_markup=cancel_keyboard(),
    )
...
def handle_restore_file(message: types.Message):
    ...
    try:
        ...
        pass
    bot.send_message(message.chat.id, "✅ Database restored.", reply_markup=admin_main_menu())
...
def menu_use_child_admin(message: types.Message):
    ...
    bot.send_message(
        message.chat.id,
        "🎛 <b>Open Child Bot Admin Panel</b>\n\nSelect a bot:",
    ...
    try:
        bot.edit_message_text(
            ...
            text=(
                ...
                "Your Telegram ID is pre-authorized as admin."
            ),
            parse_mode="HTML",
...
def handle_add_admin(message: types.Message):
    ...
    if db.is_admin(new_id):
        ...
        bot.send_message(
            message.chat.id, f"✅ <code>{new_id}</code> added as admin.", reply_markup=admin_main_menu()
        )
    cs(uid)
    ...
    if db.remove_admin(target):
        try:
            bot.edit_message_text(
                call.message.chat.id,
                call.message.message_id,
                text=f"✅ Admin <code>{target}</code> removed.",
                parse_mode="HTML",
            )
        except Exception:
            pass
    else:
        bot.send_message(call.message.chat.id, "⚠️ Cannot remove owner or admin not found.")
...
def menu_switch_db(message: types.Message):
    ...
    uri_disp = ""
    if db_type == "mongodb" and info.get("mongo_uri"):
        ...
        uri_disp = f"\n🔗 URI: <code>{masked[:70]}</code>"

    bot.send_message(
        message.chat.id,
        f"🗄 <b>Database Settings</b>\n\n"
        f"📊 Current: <b>{icon}</b>{uri_disp}\n\n"
...
def cb_switch_to_mongodb(call: types.CallbackQuery):
    ...
    bot.send_message(
        ...
        "<code>mongodb+srv://user:pass@cluster.mongodb.net/dbname</code>\n"
        "or\n"
        "<code>mongodb://localhost:27017/botmanager</code>\n\n"
        "Free cloud DB: <a href='https://www.mongodb.com/cloud/atlas'>MongoDB Atlas</a>",
        reply_markup=cancel_keyboard(),
        disable_web_page_preview=True,
    )
...
def handle_mongo_uri(message: types.Message):
    ...
    if not uri.lower().startswith("mongodb"):
        bot.reply_to(
            message,
            "⚠️ Invalid URI. Must start with <code>mongodb://</code> or <code>mongodb+srv://</code>",
        )
        return
    ...
    try:
        bot.edit_message_text(
            ...
            text=(
                ...
                "Config saved. <b>Restart the admin bot</b> to apply fully.\n\n"
                "💡 Use <b>🔁 Migrate data now</b> in the Switch Database menu to copy data to MongoDB."
            ),
            parse_mode="HTML",
        )
    except Exception:
        pass
    bot.send_message(message.chat.id, "✅ Database switched to MongoDB.", reply_markup=admin_main_menu())
...
def cb_switch_to_sqlite(call: types.CallbackQuery):
    ...
    try:
        ...
        bot.send_message(call.message.chat.id, "✅ Switched to SQLite. Please restart the bot.")


@bot.callback_query_handler(func=lambda c: c.data == "migrate_data")
@require_owner_cb
def cb_migrate_data(call: types.CallbackQuery):
    bot.answer_callback_query(call.id, "⏳ Starting migration…")
    if db_config.get_db_type() != "mongodb":
        bot.send_message(
            call.message.chat.id, "⚠️ Switch to MongoDB first, then migrate."
        )
        return
    ...
    try:
        ...
        try:
            bot.edit_message_text(
                call.message.chat.id, status_msg.message_id,
                text=f"❌ Migration error: <code>{e}</code>",
                parse_mode="HTML",
            )
        except Exception:
            pass
...
def menu_server_status(message: types.Message):
    ...
    if db_type == "sqlite":
        ...
        lines.append("🗃 <b>Database:</b> SQLite (Local)")
        lines.append(f"📁 Location: <code>{data_dir}</code>")
        lines.append(f"📦 Files: {len(db_files)} ({total_size:.1f} KB)")
        lines.append("✅ Status: Active")
    else:
        uri = info.get("mongo_uri", "")
        lines.append("🍃 <b>Database:</b> MongoDB (Cloud)")
        try:
            from shared.mongo_db import test_connection
            ok, err = test_connection(uri)
            lines.append(f"{'✅ Status: Connected' if ok else '❌ Status: Error — ' + err[:80]}")
        except Exception as e:
...
def startup():
    ...
    for b in db.list_child_bots():
        try:
            ...
            pass
    logger.info(
        "Admin Bot ready. DB: %s | Owner: %s",
        db_config.get_db_type(), OWNER_ID,
    )
